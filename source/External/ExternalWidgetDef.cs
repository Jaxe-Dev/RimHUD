...
namespace RimHUD
{
  public abstract class ExternalWidgetDef : Def
  {
    ...
    protected readonly struct ExternalMethodHandler<T>
    {
      private readonly FastInvokeHandler _handler;

      public ExternalMethodHandler(FastInvokeHandler handler) => _handler = handler;

      public T Invoke(params object[] parameters)
      {
        try { return (T)_handler.Invoke(null, parameters); }
        catch (Exception exception) { throw exception.AddData($"Invoking method '{FastInvokeHandlerMethodNameRegex.Match(_handler.Method.Name).Value ?? "<unknown>"}' threw an exception: ", true); }
      }
    }
  }
}
