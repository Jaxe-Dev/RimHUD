...
namespace RimHUD.Interface.Hud.Models.Selectors
{
  public sealed class OutfitSelector : SelectorModel
  {
    ...
    {
      if (!Active.Pawn.IsPlayerFaction() || Active.Pawn.outfits?.CurrentOutfit is null) { return; }

      Label = Lang.Get("Model.Selector.OutfitFormat", Active.Pawn.outfits?.CurrentOutfit.label!);
    ...
    private static void DrawFloatMenu()
    {
      try
      {
        ...
        options.Add(new FloatMenuOption(Lang.Get("Model.Selector.Manage").Italic(), static () => Find.WindowStack!.Add(new Dialog_ManageOutfits(Active.Pawn.outfits!.CurrentOutfit))));
