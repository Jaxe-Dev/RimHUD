...
namespace RimHUD.Interface.Hud.Models
{
  public class PawnModel
  {
    ...
    public HealthConditionModel Health { get; }
    public MindConditionModel Mind { get; }
    ...
    public IModelSelector OutfitSelector => new OutfitModel(this);
    public IModelSelector FoodSelector => Mod_PawnRules.Instance.IsActive ? RulesSelector : new FoodModel(this);
    public IModelSelector RulesSelector => Mod_PawnRules.Instance.IsActive ? new Mod_PawnRules.ModelRules(this) : null;
    public IModelSelector TimetableSelector => new TimetableModel(this);
    public IModelSelector AreaSelector => new AreaModel(this);

    private PawnModel(Pawn pawn)
    {
      Base = pawn;
      Health = new HealthConditionModel(this);
      Mind = new MindConditionModel(this);
    ...
    {
      if (simple || !ModsConfig.IdeologyActive || !(Base.RaceProps?.Humanlike ?? true)) { return Base.kindDef?.race?.label; }
    ...
    private string GetActivity()
    {
      try
      {
        ...
        var target = Base.IsAttacking() ? Base.TargetCurrentlyAimingAt.Thing?.LabelCap : null;
    ...
    private string GetBioTooltip()
    {
      ...
      HudBuilder.BuildStatString(Base, builder, StatDefOf.PsychicSensitivity);
      ...
      if (ModsConfig.RoyaltyActive)
      {
        ...
        HudBuilder.BuildStatString(Base, builder, StatDefOf.MeditationFocusGain);
        HudBuilder.BuildStatString(Base, builder, StatDefOf.MeditationFocusStrength);
      }
      if (ModsConfig.IdeologyActive)
      {
        HudBuilder.BuildStatString(Base, builder, StatDefOf.SocialIdeoSpreadFrequencyFactor);
        HudBuilder.BuildStatString(Base, builder, StatDefOf.CertaintyLossFactor);
      }
