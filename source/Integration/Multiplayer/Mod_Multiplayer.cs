  ...
  public sealed class Mod_Multiplayer : IntegratedMod
  {
    public Mod_Multiplayer() : base("Multiplayer", "0MultiplayerAPI", "Multiplayer.API.MP")
    {
      try
      {
        ...
        var registerSyncMethod = Traverse.Method("RegisterSyncMethod", new[] { typeof(Type), typeof(string), AccessTools.TypeByName("Multiplayer.API.SyncType")?.MakeArrayType() ?? throw new Exception("Failed to find RegisterSyncMethod argument types.") });
