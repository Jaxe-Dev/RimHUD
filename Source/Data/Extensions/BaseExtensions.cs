...
namespace RimHUD.Data.Extensions
{
    internal static class BaseExtensions
    {
        ...
        public static string ToDecimalString(this int self, int remainder) => !Theme.Theme.ShowDecimals.Value ? self.ToString().Bold() : $"{self.ToString().Bold()}.{(remainder >= 100 ? "99" : remainder.ToString("D2")).Size(Theme.Theme.SmallTextStyle.ActualSize)}";
